...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'serviceservice')))

from api import get_customers, get_interaction

def load_data():
    customers_df = pd.DataFrame(get_customers())
    interact_df = pd.DataFrame(get_interaction())
    
    return customers_df, interact_df

def overview_tab(customers_df, interactions_df):
...
def customer_analysis_tab(customers_df, interactions_df):
    ...
    if not interactions_df.empty and 'customer_id' in interactions_df.columns:
        ...
        if not customers_df.empty:
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    automargin=True,
                    range=[0, max(engagement_by_type['interactions']) * 1.2] 
                ),
            )
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                automargin=True,
                range=[0, max(top_customers['interactions']) * 1.2] 
            ),
        )
        fig.update_traces(textposition='outside')
        st.plotly_chart(fig, use_container_width=True)

def interaction_analysis_tab(interactions_df):
        ...
        if 'type' in interactions_df.columns:
            ...
            with col2:
                ...
                fig_bar.update_layout(
                    ...
                    yaxis=dict(
                        automargin=True,
                        range=[0, max(interaction_types['Count']) * 1.2] 
                    ),
                )
            ...
            if 'date' in interactions_df.columns or 'timestamp' in interactions_df.columns:
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(
                        automargin=True,
                        range=[0, max(day_counts['Count']) * 1.2] 
                    ),
                )
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)
...
def report_page():
    ...
    customers_df, interactions_df = load_data()
    if customers_df is None or interactions_df is None:
        return
    ...
    with tab1:
        overview_tab(customers_df, interactions_df)
    
    with tab2:
        customer_analysis_tab(customers_df, interactions_df)
    
    with tab3:
        interaction_analysis_tab(interactions_df) 