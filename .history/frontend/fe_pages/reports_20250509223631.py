...
def report_page():
    ...
    height=450  # 👈 (1) Thêm chiều cao cố định
)

fig_bar.update_layout(
    margin=dict(l=50, r=50, t=50, b=100),  # 👈 (2) Tăng margin bottom từ 0 lên 100
    yaxis=dict(
        automargin=True,
        title_standoff=15,  # 👈 (3) Tăng khoảng cách tiêu đề trục
        range=[0, max(customer_summary['Customer Number']) * 1.15]  # 👈 (4) Tự động điều chỉnh phạm vi trục y
    ),
    showlegend=False,
    uniformtext_minsize=8,  # 👈 (5) Đặt kích thước chữ tối thiểu
    uniformtext_mode='hide'  # 👈 (6) Ẩn text nếu không đủ chỗ
)

fig_bar.update_traces(
    textposition='outside',
    textfont_size=14,  # 👈 (7) Tăng kích thước chữ
    textangle=0,  # 👈 (8) Đảm bảo text không bị xoay
    cliponaxis=False  # 👈 (9) Quan trọng: Cho phép text vượt ra ngoài khung
)
