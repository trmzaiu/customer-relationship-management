...
def customer_page():
    ...
    if "delete_state" not in st.session_state:
        st.session_state.delete_state = "initial"  # Các trạng thái: initial, confirm, processing, done
    
    confirm_delete = st.text_input("Type 'DELETE' to confirm", placeholder="DELETE", key="confirm_delete")
    
    # Hiển thị thông tin debug
    st.write(f"Current state: {st.session_state.delete_state}")
    ...
    if st.session_state.delete_state == "confirm":
        st.warning("Bạn đã bấm nút xóa!")
        
        if confirm_delete != "DELETE":
            st.error("Vui lòng nhập 'DELETE' để xác nhận")
        else:
            st.session_state.delete_state = "processing"
            st.experimental_rerun()
    
    elif st.session_state.delete_state == "processing":
        st.info("Đang xử lý yêu cầu xóa...")
        
        try:
            delete_url = f"{CUSTOMER_API_URL}/{lookup_id}"
            st.write(f"Sending DELETE request to: {delete_url}")
            
            res = requests.delete(delete_url)
            st.write(f"DELETE response: {res.status_code}")
            
            if res.status_code == 200:
                st.session_state.delete_state = "done"
                st.success("✅ Customer deleted successfully!")
                if 'found_customer' in st.session_state:
                    del st.session_state['found_customer']
            else:
                st.error(f"❌ Failed to delete customer: {res.text}")
                st.session_state.delete_state = "initial"
        except Exception as e:
            st.error(f"🚨 Connection error: {str(e)}")
            st.session_state.delete_state = "initial"
    
    elif st.session_state.delete_state == "done":
        st.success("Customer was deleted successfully!")
        
        if st.button("OK", key="reset_delete"):
            st.session_state.delete_state = "initial"
            st.experimental_rerun()
                else:
