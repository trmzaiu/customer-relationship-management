...
def customer_page():
    ...
    with tab2:
        ...
        with col1:
            cust_id = st.text_input("Customer ID (Optional)", key="new_id", disabled=True, placeholder= length + 1)
...
def check_email_exists(api_url, email_to_check):
    """
    Check if an email exists in the customer API data
    
    Args:
        api_url (str): The URL of the customer API
        email_to_check (str): The email to search for
    
    Returns:
        tuple: (exists: bool, customer_data: dict/None)
               - exists: True if email exists, False otherwise
               - customer_data: The customer data if found, None otherwise
    """
    try:
        # Make API request
