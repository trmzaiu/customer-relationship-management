...
def dashboard_page(auto_refresh=False):
    ...
    # Customer Distribution and Analytics
    st.markdown("---")
    ...
    with col2:
        ...
        # Let's create some example data for customer growth if datetime exists
        if 'datetime' in customers_df.columns:
            customers_df['date'] = pd.to_datetime(customers_df['datetime']).dt.date
            try:
                # First try ISO8601 format (handles most cases including timezones)
                customers_df['datetime'] = pd.to_datetime(customers_df['datetime'], format='ISO8601', errors='coerce')
                # Then fallback to mixed format if needed
                if customers_df['datetime'].isna().any():
                    customers_df['datetime'] = pd.to_datetime(customers_df['datetime'], format='mixed', errors='coerce')
                # Format for display
                customers_df['datetime'] = customers_df['datetime'].dt.strftime('%b %d, %Y %H:%M')
            except Exception as e:
                st.warning(f"Could not format datetime column: {str(e)}")
                # Keep original values if formatting fails
                customers_df['datetime'] = customers_df['datetime'].astype(str)
    ...
    if not interact_df.empty and 'type' in interact_df.columns:
        # Sort by date if date column exists and has valid values
