...
def interaction_page():
    ...
    if 'Customer' in df.columns:
        ...
        # Get customer names (assuming check_customer_id_exist returns a dict with 'name')
        customer_names = []
        for customer_id in df['Customer']:
            _, customer_data = check_customer_id_exist(customer_id)
            customer_names.append(customer_data.get('name', 'Unknown'))
    ...
    if 'Date' in df.columns:
        df['Date'] = data['date']  # Fallback to original date values
    if 'Customer' in df.columns:
        df['Customer'] = data['customer_id']  # Fallback to original IDs
