        ...
        if st.button("🔍 Find Customer", key="find_customer", use_container_width=True):
            try:
                ...
                if res.ok:
                    ...
                    with delete_tab:
    # 👇 Khai báo một container để hiển thị thông báo
    delete_status = st.empty()  
    
    confirm_delete = st.text_input("Type 'DELETE' to confirm", key="confirm_delete")
    
    if st.button("🗑️ Delete Customer", key="delete_btn", use_container_width=True):
        if confirm_delete != "DELETE":
            delete_status.error("⚠️ Vui lòng nhập 'DELETE' để xác nhận!")
        else:
            # 👇 Hiển thị thông báo "Đang xóa..." ngay lập tức
            delete_status.info("🔄 Đang xóa khách hàng...")
            ...
            try:
                ...
                # ✅ Xử lý kết quả
                if res.ok:
                    delete_status.success("✅ Đã xóa khách hàng thành công!")
                    ...
                    delete_status.error(f"❌ Lỗi: {res.status_code} - {res.text}")
            
            except Exception as e:
                delete_status.error(f"🚨 Lỗi kết nối: {str(e)}")
                else:
