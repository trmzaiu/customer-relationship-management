...
def dashboard_page(auto_refresh=True):
    ...
    if not interact_df.empty and 'type' in interact_df.columns:
        ...
        for _, activity in recent_activities.iterrows():
            ...
            with col2:
                ...
                _, customer = check_customer_id_exist(activity['customer_id'])
                st.markdown(f"""
                <div style='border-left:1px solid #eee; padding-left:15px;'>
                    <div style='font-weight:500;'>{activity['type']} with {customer['name']}</div>
