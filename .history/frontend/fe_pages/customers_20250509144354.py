        ...
        if 'customer_data' in st.session_state:
            ...
            if not data:
                ...
                if view_type == "Table":
                    ...
                    # Đảm bảo page_number luôn là integer và trong phạm vi hợp lệ
                    page_number = int(st.session_state.get('current_page', 1))
                    page_number = max(1, min(page_number, total_pages))
                    # Trong phần phân trang của customer_page()
                    try:
                        # Lấy giá trị page_number từ session state, mặc định là 1
                        page_number = st.session_state.get('current_page', 1)
                        
                        # Đảm bảo luôn là integer hợp lệ
                        page_number = int(page_number) if str(page_number).isdigit() else 1
                        page_number = max(1, page_number)  # Đảm bảo không nhỏ hơn 1
                        
                        # Tính toán số trang
                        items_per_page = 10
                        total_pages = max(1, math.ceil(len(data) / items_per_page))
                        
                        # Giới hạn page_number không vượt quá total_pages
                    ...
                    with col2:
                        pagination_html = """
                        <div class="pagination" style="display: flex; justify-content: center; gap: 5px; margin: 15px 0;">
                        """
                        
                        # Nút Previous
                        if page_number > 1:
                            pagination_html += f"""
                            <a href="#" onclick="streamlitSessionState.set('current_page', {page_number-1}); streamlitRerun()">
                                &laquo; Prev
                            </a>
                            """
                        ...
                        for i in range(1, total_pages + 1):
                            active = "background: #0d6efd; color: white;" if i == page_number else ""
                            pagination_html += f"""
                            <a href="#" onclick="streamlitSessionState.set('current_page', {i}); streamlitRerun()" 
                            style="padding: 5px 10px; border-radius: 5px; border: 1px solid #ddd; {active}">
                                {i}
                            </a>
                            """
