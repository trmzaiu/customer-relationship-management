...
def customer_page():
    ...
    with tab4:
        ...
        template_type = st.radio("Format:", ["CSV", "Excel"], horizontal=True)
        
        if st.button("⬇️ Download Template File"):
            ...
            if template_type == "CSV":
                ...
                href = f'<a href="data:file/csv;base64,{b64}" download="customer_import_template.csv" class="button">Download CSV Template</a>'
                st.markdown(href, unsafe_allow_html=True)
            else:
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                    sample_df.to_excel(writer, sheet_name='Customers', index=False)
                buffer.seek(0)
                b64 = base64.b64encode(buffer.getvalue()).decode()
                href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="customer_import_template.xlsx" class="button">Download Excel Template</a>'
                st.markdown(href, unsafe_allow_html=True)
    ...
    with st.expander("ℹ️ Help & Information"):
        ...
        **Using the Customer Management Dashboard:**
        ...
        - **Customer List**: View all customers with filtering options
        - **Add Customer**: Create new customer records
        - **Search & Modify**: Find, update, or delete existing customers
        - **Import Data**: Upload CSV or Excel files to bulk import customers
