from frontend.fe_pages.widget import CUSTOMER_API_URL
...
def dashboard_page(auto_refresh=False):
    ...
    if 'date' not in interact_df.columns:
        interact_df['date'] = pd.to_datetime(interact_df['timestamp'] if 'timestamp' in interact_df.columns else interact_df.get('date', None), errors='coerce')
        ...
        for _, activity in recent_activities.iterrows():
            ...
            with col2:
                ...
                <div style='border-left:1px solid #eee; padding-left:15px;'>
                    ...
                    <div style='color:#999; font-size:0.8em;'>{activity['date']}</div>
                </div>
