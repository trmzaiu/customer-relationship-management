            ...
            try:
                ...
                if res.ok:
                    ...
                    with delete_tab:
                        ...
                        # Debug info
                        st.write(f"Confirm delete value: '{confirm_delete}'")
                        
                        delete_button = st.button("🗑️ Delete Customer", key="delete", use_container_width=True)
                        
                        if delete_button:
                            st.warning("Nút delete đã được bấm")
                            
                            if confirm_delete != "DELETE":
                                st.error(f"Please type 'DELETE' to confirm. Current value: '{confirm_delete}'")
                            else:
                                st.info("Điều kiện xác nhận đã đúng, đang gửi yêu cầu DELETE...")
                                try:
                                    ...
                                    if res.status_code == 200:
                                        ...
                                        # Tạm thời bỏ dòng này để xem tất cả thông báo
                                        # st.experimental_rerun()
                                    else:
                                        st.error(f"❌ Failed to delete customer. Status: {res.status_code}, Error: {res.text}")
                                except Exception as e:
