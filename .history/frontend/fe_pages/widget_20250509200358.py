...
def load_interaction_data():
    with st.spinner('Đang tải dữ liệu tương tác...'):
        ...
        if res.status_code == 200:
            st.session_state.interaction_data = res.json()
            
            # Pre-cache thông tin khách hàng
            if 'customer_cache' not in st.session_state:
                st.session_state.customer_cache = {}
                
            for item in st.session_state.interaction_data:
                customer_id = item['customer_id']
                if customer_id not in st.session_state.customer_cache:
                    _, customer_data = check_customer_id_exist(customer_id)
                    st.session_state.customer_cache[customer_id] = customer_data
        else:
